...
const AddMember = () => {
    ...
    const initialValues = {
        name: '',
        positionDetails: { position: '', departmentId: '' }, // Sử dụng đối tượng để lưu position và departmentId
        yearOfBirth: '',
        qualification: '',
        seniority: '',
        image: null,
    ...
    useEffect(() => {
        const fetchDepartments = async () => {
            try {
                ...
                // Ánh xạ giá trị phòng ban với chỉ số cộng thêm 1
                const mappedDepartments = fetchedDepartments.map((department, index) => ({
                    ...department,
                    displayValue: (index + 1).toString(), // Ánh xạ thành số 1, 2, 3, ...
                }));
    ...
    const handleSubmit = async (values, { resetForm }) => {
        ...
        formData.append('position', values.positionDetails.position); // Truyền giá trị position
        formData.append('departmentId', values.positionDetails.departmentId); // Truyền departmentId
        ...
        try {
            await addMember(formData, values.positionDetails.departmentId); // Truyền departmentId vào hàm addMember
